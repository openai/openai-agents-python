...
async def main(model: str, question: str) -> None:
    ...
    sandbox_reviewer = SandboxAgent(
        ...
        model=model,
        # `instructions` is the base agent instructions for the review-and-handoff task.
        instructions=(
            "You inspect onboarding documents in the sandbox, verify the facts, then hand off "
            "to the account executive assistant to draft the final note."
        ),
        # `developer_instructions` is appended after that as additional deterministic instructions.
        # Here, "do not answer directly" is kept in `developer_instructions`.
        developer_instructions="Do not answer the user directly after reviewing the packet.",
