...
def _require_aliyun_credentials(
    *,
    ...
    del access_key_id, access_key_secret, api_key  # resolved by the SDK, not preflighted here.
    # The one hard requirement is the account id: AgentRun builds the data-plane
    # endpoint from it (unless a data endpoint is set). Access credentials themselves
    # are left to `agentrun-sdk`, which resolves them from CLI flags, env vars, an
    # Alibaba Cloud config file, or a RAM role — so we do not second-guess them here.
    env = os.environ
    has_account = bool(
        account_id
        or env.get("AGENTRUN_ACCOUNT_ID")
        or env.get("FC_ACCOUNT_ID")
        or env.get("AGENTRUN_DATA_ENDPOINT")
    )
    if not has_account:
        raise SystemExit(
            "AgentRun needs an account id to build the data-plane endpoint. Set "
            "AGENTRUN_ACCOUNT_ID (or FC_ACCOUNT_ID / AGENTRUN_DATA_ENDPOINT), or pass "
            "--account-id."
        )
