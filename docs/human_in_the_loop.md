...
- **Hosted MCP servers**: Set `require_approval` to `"always"` on `HostedMCPTool` to force HITL, optionally providing `on_approval_request` to auto-approve or reject (see `examples/hosted_mcp/human_in_the_loop.py`, `examples/hosted_mcp/on_approval.py`, and `examples/hosted_mcp/policy_based_approval.py`). Use `"never"` for trusted servers (`examples/hosted_mcp/simple.py`).
