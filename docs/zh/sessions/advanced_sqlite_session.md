...
`AdvancedSQLiteSession` 是基础 `SQLiteSession` 的增强版，提供高级会话管理能力，包括会话分支、详细使用分析以及结构化会话查询。

## 功能

- **会话分支**: 可从任意用户消息创建替代会话路径
- **使用跟踪**: 每轮对话的详细 token 使用分析，并提供完整 JSON 拆解
- **结构化查询**: 按轮次获取会话、工具使用统计等
- **分支管理**: 独立的分支切换与管理
- **消息结构元数据**: 跟踪消息类型、工具使用和会话流向
...
- `logger` (logging.Logger | None): 会话的自定义日志记录器。默认使用模块日志器
...
AdvancedSQLiteSession 的关键特性之一是能够从任意用户消息创建会话分支，从而探索替代的会话路径。
...
AdvancedSQLiteSession 提供多种方法来分析会话结构和内容。
