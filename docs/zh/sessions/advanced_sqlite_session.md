...
- **使用情况追踪**: 每轮对话的详细 token 使用分析，附完整 JSON 拆解
...
- `logger` (logging.Logger | None): 会话的自定义日志记录器。默认使用模块级 logger

## 使用情况追踪

AdvancedSQLiteSession 通过为每轮对话存储 token 使用数据来提供详细的使用分析。**这完全依赖于在每次智能体运行后调用 `store_run_usage` 方法。**
...
AdvancedSQLiteSession 的关键特性之一是能够从任意用户消息创建对话分支，便于探索替代的对话路径。
...
查看[完整示例](https://github.com/openai/openai-agents-python/tree/main/examples/memory/advanced_sqlite_session_example.py)，以获取对所有功能的全面演示。
