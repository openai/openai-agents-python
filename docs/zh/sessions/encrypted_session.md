...
`EncryptedSession` 为任意会话实现提供透明加密，通过自动过期机制保护会话数据并清理过期条目。

## 功能

- **透明加密**：使用 Fernet 加密封装任何会话
- **会话级密钥**：通过 HKDF 派生每个会话唯一的加密密钥
- **自动过期**：当 TTL 过期时静默跳过旧条目
- **即插即用**：可与任何现有会话实现配合使用
...
!!! warning "高级会话特性"
    ...
    - 由于消息内容已被加密，诸如 `find_turns_by_content()` 等方法将无法有效工作
    - 基于内容的搜索会在加密数据上运行，从而限制其效果
