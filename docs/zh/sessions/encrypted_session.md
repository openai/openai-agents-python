...
`EncryptedSession` 为任意会话实现提供透明加密，通过自动过期机制保障对话数据安全。

## 功能

- **透明加密**: 使用 Fernet 加密封装任意会话
- **每会话唯一密钥**: 通过 HKDF 为每个会话派生唯一加密密钥
...
!!! warning "高级会话功能"
    ...
    - 由于消息内容已加密，`find_turns_by_content()` 等方法将难以有效工作
    - 基于内容的搜索会作用在加密数据上，其有效性受限
