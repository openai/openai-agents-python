...
1. 你创建任意需要的 Python 对象。常见模式是使用 dataclass 或 Pydantic 对象。
...
需要特别注意的**最重要**的一点：对某次智能体运行而言，所有智能体、工具函数、生命周期等必须使用相同类型的上下文。
...
!!! danger "注意"
    ...
    上下文对象**不会**发送给 LLM。它完全是一个本地对象，你可以读取、写入并在其上调用方法。
...
2. 这是一个工具。你可以看到它接收 `RunContextWrapper[UserInfo]`。该工具的实现从上下文中读取数据。
3. 我们为智能体标注了泛型 `UserInfo`，以便类型检查器捕获错误（例如，如果我们尝试传入一个使用不同上下文类型的工具）。
4. 将上下文传给 `run` 函数。
5. 智能体正确调用工具并获得年龄。
...
当调用 LLM 时，它能够看到的**唯一**数据来自会话历史。因此，如果你希望让某些新数据对 LLM 可见，必须以能使其出现在历史中的方式提供。有几种方法：

1. 你可以将其添加到智能体的 `instructions`。这也被称为“系统提示词”或“开发者消息”。系统提示词可以是静态字符串，也可以是接收上下文并输出字符串的动态函数。这对总是有用的信息很常见（例如用户名或当前日期）。
2. 在调用 `Runner.run` 函数时将其添加到 `input`。这类似于 `instructions` 的做法，但允许你添加在[指挥链](https://cdn.openai.com/spec/model-spec-2024-05-08.html#follow-the-chain-of-command)中更靠后的消息。
3. 通过 工具调用 暴露它。这对按需的上下文很有用——LLM 会决定何时需要某些数据，并可调用工具来获取这些数据。
4. 使用检索或 网络检索。它们是能够从文件或数据库（检索）或从网络（网络检索）获取相关数据的特殊工具。这有助于用相关的上下文数据来“落地”响应。