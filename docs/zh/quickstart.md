...
可以以相同方式定义其他智能体。`handoff_descriptions` 为确定任务转移路由提供额外上下文。
...
让我们检查工作流是否运行，以及分诊智能体是否能在两个专家智能体之间正确路由。
...
让我们把以上全部整合起来，使用任务转移和输入安全防护措施运行整个工作流。
...
要回顾智能体运行期间发生的事情，请前往 OpenAI Dashboard 的 [Trace viewer](https://platform.openai.com/traces) 查看运行追踪。
