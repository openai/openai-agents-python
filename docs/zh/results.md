...
二者都继承自 [`RunResultBase`][agents.result.RunResultBase]，大多数有用的信息都在这里。
...
!!! note
    ...
    `final_output` 的类型是 `Any`。由于 任务转移，我们无法进行静态类型标注。如果发生 任务转移，意味着任意智能体都可能成为最后一个，因此我们在静态上无法知道可能的输出类型集合。

## 下一轮输入

你可以使用 [`result.to_input_list()`][agents.result.RunResultBase.to_input_list] 将结果转换为输入列表，它会把你提供的原始输入与智能体运行期间生成的各项内容拼接起来。这样便于将一次智能体运行的输出传入另一次运行，或在循环中每次追加新的用户输入。

## 最后一个智能体

[`last_agent`][agents.result.RunResultBase.last_agent] 属性包含最后一个运行的智能体。根据你的应用，这通常在下次用户输入时很有用。比如，如果你有一个前线分诊智能体会 任务转移 到特定语言的智能体，你可以存储该最后智能体，并在下一次用户向智能体发送消息时复用它。
...
工具安全防护措施结果单独提供为 [`tool_input_guardrail_results`][agents.result.RunResultBase.tool_input_guardrail_results] 和 [`tool_output_guardrail_results`][agents.result.RunResultBase.tool_output_guardrail_results]。这些安全防护措施可附加到工具上，并在智能体工作流中的工具调用期间执行。
...
[`input`][agents.result.RunResultBase.input] 属性包含你传递给 `run` 方法的原始输入。多数情况下你不需要它，但在需要时可以使用。