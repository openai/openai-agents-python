...
- **total_tokens**: 输入 + 输出
- **request_usage_entries**: 每次请求的用量拆分列表
- **details**:
...
在 `Runner.run(...)` 之后，通过 `result.context_wrapper.usage` 获取用量。
...
用量会在该次运行中的所有模型调用间聚合（包括工具调用和任务转移）。

### 在 LiteLLM 模型中启用用量

LiteLLM 提供方默认不报告用量指标。当你使用 [`LitellmModel`](models/litellm.md) 时，向你的智能体传入 `ModelSettings(include_usage=True)`，这样 LiteLLM 的响应会填充到 `result.context_wrapper.usage`。
...
SDK 会自动在 `request_usage_entries` 中跟踪每次 API 请求的用量，便于进行细粒度的成本计算和监控上下文窗口的消耗。
...
请注意，虽然会话会在运行之间保留对话上下文，但每次 `Runner.run()` 返回的用量指标仅代表该次执行。在会话中，以前的消息可能会作为输入重新传入每次运行，从而影响后续轮次的输入 token 计数。
