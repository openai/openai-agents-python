...
用量会在整个运行期间聚合所有模型调用（包括工具调用和任务转移）。
...
SDK 会自动在 `request_usage_entries` 中追踪每个 API 请求的用量，便于精细的成本计算和监控上下文窗口消耗。
...
请注意，虽然会话在多次运行之间保留对话上下文，但每次 `Runner.run()` 返回的用量指标仅代表该次执行本身。在会话中，先前消息可能会作为输入重新提供给每次运行，从而影响后续轮次的输入 token 计数。
