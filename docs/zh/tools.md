...
工具让智能体可以执行操作：例如获取数据、运行代码、调用外部 API，甚至进行计算机操作。在 Agents SDK 中有三类工具：

- 托管工具：这些工具在 LLM 服务与 AI 模型并行运行。OpenAI 提供检索、网络检索和计算机操作作为托管工具。
- 工具调用：这些允许你将任意 Python 函数用作工具。
- 将智能体作为工具：这允许你将智能体用作工具，使智能体能够调用其他智能体而无需进行任务转移。
...
- 工具名称将是 Python 函数名（或你可以提供名称）
- 工具描述将取自函数的 docstring（或你可以提供描述）
- 函数输入的 schema 将根据函数参数自动创建
- 每个输入的描述将从函数的 docstring 中提取，除非禁用

我们使用 Python 的 `inspect` 模块提取函数签名，配合使用 [`griffe`](https://mkdocstrings.github.io/griffe/) 解析 docstring，并使用 `pydantic` 生成 schema。
...
2. 如果存在 docstring，我们会用它来捕获描述和参数说明。
3. 函数可选地接收 `context`（必须为第一个参数）。你也可以设置覆盖项，比如工具名称、描述、docstring 风格等。
4. 你可以将装饰后的函数传入工具列表。
...
除了返回文本输出外，你还可以将一个或多个图像或文件作为函数工具的输出返回。为此，你可以返回以下任意内容：
...
有时你不想将 Python 函数用作工具。此时你可以直接创建一个 [`FunctionTool`][agents.tool.FunctionTool]。你需要提供：
...
如前所述，我们会自动解析函数签名以提取工具的 schema，并解析 docstring 以提取工具及各个参数的描述。注意事项：

1. 通过 `inspect` 模块解析签名。我们使用类型注解理解参数类型，并动态构建 Pydantic 模型来表示整体 schema。它支持大多数类型，包括 Python 基本类型、Pydantic 模型、TypedDict 等。
...
在某些工作流中，你可能希望由一个中心智能体编排一组专用智能体，而不是移交控制。你可以通过将智能体建模为工具来实现。
...
- 从子智能体的聊天记录中提取特定信息（例如一个 JSON 载荷）。
...
你可以在运行时使用 `is_enabled` 参数有条件地启用或禁用智能体工具。这样可以根据上下文、用户偏好或运行时条件动态筛选 LLM 可用的工具。
...
当你通过 `@function_tool` 创建函数工具时，可以传入 `failure_error_function`。这是一个在工具调用失败时向 LLM 提供错误响应的函数。

- 默认情况下（即未传入任何值），会运行 `default_tool_error_function`，告知 LLM 发生了错误。
- 如果你传入自己的错误函数，将运行该函数并把其响应发送给 LLM。
- 如果你显式传入 `None`，则任何工具调用错误都会被重新抛出以供你处理。如果模型生成了无效 JSON，这可能是 `ModelBehaviorError`；如果你的代码崩溃，则可能是 `UserError`，等等。
...
如果你手动创建了一个 `FunctionTool` 对象，那么必须在 `on_invoke_tool` 函数内部处理错误。