...
-   With OpenAI server-managed continuation, wait for the streamed run to finish before deciding the next step. If the run completed normally, reuse the same `conversation_id`, or pass `result.last_response_id` as `previous_response_id`, together with only the new user input. If the run stopped after local tool execution but before the follow-up model call (for example after `cancel(mode="after_turn")` on a tool turn), continue that unfinished run state first instead of treating it as a clean next user turn.
...
-   Use [`RunConfig.session_input_callback`][agents.run.RunConfig.session_input_callback] only to customize how retrieved session history and the new user input are merged before the next model call. It does not compact or rewrite stored history.
