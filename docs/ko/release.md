...
이 마이너 릴리스에는 호환성을 깨는 변경 사항이 **없지만**, 주요한 새 기능 영역인 샌드박스 에이전트와 로컬, 컨테이너화 및 호스팅 환경 전반에서 이를 사용하는 데 필요한 런타임, 백엔드, 문서 지원이 추가되었습니다.

주요 변경 사항:

-   `SandboxAgent`, `Manifest`, `SandboxRunConfig`을 중심으로 한 새로운 샌드박스 런타임 인터페이스가 추가되어, 에이전트가 파일, 디렉터리, Git 저장소, 마운트, 스냅샷, 재개 지원을 갖춘 영구 격리 워크스페이스 내에서 작업할 수 있습니다.
