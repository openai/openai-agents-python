...
아직 없다면, OpenAI API 키를 만들기 위해 [이 안내](https://platform.openai.com/docs/quickstart#create-and-export-an-api-key)를 따라주세요.
...
워크플로가 실행되고 분류 에이전트(triage agent)가 두 전문 에이전트 사이를 올바르게 라우팅하는지 확인해 봅시다.
...
핸드오프와 입력 가드레일을 사용해 모든 것을 하나로 묶고 전체 워크플로를 실행해 봅시다.
...
에이전트 실행 중에 무슨 일이 일어났는지 검토하려면 [OpenAI 대시보드의 Trace viewer](https://platform.openai.com/traces)로 이동하여 실행 트레이스를 확인하세요.
