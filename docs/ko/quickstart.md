...
API 키가 없다면, OpenAI API 키를 생성하려면 [이 지침](https://platform.openai.com/docs/quickstart#create-and-export-an-api-key)을 따르세요.
...
각 에이전트에서, 작업을 진행하는 방법을 결정하기 위해 에이전트가 선택할 수 있는 아웃고잉 핸드오프 옵션 인벤토리를 정의할 수 있습니다.
...
모든 것을 합쳐서 핸드오프와 입력 가드레일을 사용하여 전체 워크플로를 실행해 봅시다.
