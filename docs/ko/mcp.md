...
Agents Python SDK는 여러 MCP 트랜스포트를 이해합니다. 이를 통해 기존 MCP 서버를 재사용하거나 직접 서버를 구축해 파일 시스템, HTTP 또는 커넥터 기반 도구를 에이전트에 노출할 수 있습니다.

## MCP 통합 선택

MCP 서버를 에이전트에 연결하기 전에 도구 호출이 어디에서 실행될지, 어떤 트랜스포트를 사용할 수 있는지 결정하세요. 아래 매트릭스는 Python SDK가 지원하는 옵션을 요약합니다.
...
| Server-Sent Events를 구현한 HTTP 서버와 통신                                         | **HTTP with SSE MCP 서버** via [`MCPServerSse`][agents.mcp.server.MCPServerSse] |
...
호스티드 툴은 전체 도구 왕복 과정을 OpenAI 인프라로 이동합니다. 코드에서 도구를 나열하고 호출하는 대신,
[`HostedMCPTool`][agents.tool.HostedMCPTool]이 서버 레이블(및 선택적 커넥터 메타데이터)을 Responses API로 전달합니다. 모델은 원격 서버의 도구를 나열하고 추가적인 Python 프로세스 콜백 없이 이를 호출합니다. 호스티드 툴은 현재 Responses API의 호스티드 MCP 통합을 지원하는 OpenAI 모델에서 작동합니다.

### 기본 호스티드 MCP 도구

에이전트의 `tools` 리스트에 [`HostedMCPTool`][agents.tool.HostedMCPTool]을 추가하여 호스티드 툴을 생성합니다. `tool_config`
dict는 REST API에 전송할 JSON과 동일한 구조를 반영합니다:
...
`require_approval`을 단일 정책(`"always"`, `"never"`) 또는 도구 이름을 정책에 매핑한 dict로 구성하세요. Python 내부에서 결정을 내리려면 `on_approval_request` 콜백을 제공합니다.
...
콜백은 동기 또는 비동기로 구현할 수 있으며, 모델이 계속 실행되기 위해 승인 데이터가 필요할 때마다 호출됩니다.
...
- `tool_filter`는 노출할 도구의 하위 집합만 선택하도록 허용합니다 ([도구 필터링](#tool-filtering) 참조)
...
프로세스를 스폰하고 파이프를 열어 두며 컨텍스트 매니저가 종료될 때 자동으로 닫습니다. 이 옵션은 빠른 프로토타입 제작 또는 서버가 명령줄 엔트리 포인트만 노출할 때 유용합니다.
...
[`create_static_tool_filter`][agents.mcp.create_static_tool_filter]를 사용해 간단한 허용/차단 리스트를 구성하세요:
...
`allowed_tool_names`와 `blocked_tool_names`가 모두 제공되면 SDK는 허용 리스트를 먼저 적용한 뒤 남은 집합에서 차단된 도구를 제거합니다.

### 동적 도구 필터링

더 정교한 로직이 필요하면 [`ToolFilterContext`][agents.mcp.ToolFilterContext]를 인수로 받는 호출 가능한 객체를 전달하세요. 이 호출 객체는 동기 또는 비동기일 수 있으며, 도구를 노출해야 하면 `True`를 반환합니다.
...
MCP 서버는 에이전트 instructions를 동적으로 생성하는 프롬프트를 제공할 수도 있습니다. 프롬프트를 지원하는 서버는 두 가지
메서드를 노출합니다:
...
모든 에이전트 실행은 각 MCP 서버에 대해 `list_tools()`를 호출합니다. 원격 서버는 눈에 띄는 지연을 유발할 수 있으므로, 모든 MCP
