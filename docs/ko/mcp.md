...
Agents Python SDK는 여러 MCP 전송 방식을 지원합니다. 이를 통해 기존 MCP 서버를 재사용하거나, 자체 서버를 구축해 파일 시스템, HTTP, 커넥터 기반 도구를 에이전트에 노출할 수 있습니다.

## MCP 통합 선택

에이전트에 MCP 서버를 연결하기 전에, 도구 호출이 어디에서 실행될지와 접근 가능한 전송 방식을 결정하세요. 아래 매트릭스는 Python SDK가 지원하는 옵션을 요약합니다.

| 필요한 것                                                                            | 권장 옵션                                              |
| ------------------------------------------------------------------------------------ | ------------------------------------------------------ |
| 모델을 대신해 OpenAI의 Responses API가 공용 접근 가능한 MCP 서버를 호출하도록 함      | **호스티드 MCP 서버 도구** via [`HostedMCPTool`][agents.tool.HostedMCPTool] |
...
[`HostedMCPTool`][agents.tool.HostedMCPTool]이 서버 레이블(및 선택적 커넥터 메타데이터)을 Responses API로 전달합니다. 모델은 원격 서버의 도구를 나열하고, 추가적인 Python 프로세스 콜백 없이 이를 호출합니다. 호스티드 툴은 현재 Responses API의 호스티드 MCP 통합을 지원하는 OpenAI 모델에서 동작합니다.
...
모델이 동작 중일 때 점진적으로 MCP 출력을 소비하세요:
...
서버가 민감한 작업을 수행할 수 있는 경우, 각 도구 실행 전에 수동 또는 프로그램적 승인을 요구할 수 있습니다. `tool_config`의
`require_approval`을 단일 정책(`"always"`, `"never"`) 또는 도구 이름을 정책에 매핑한 dict로 구성하세요. Python 내부에서 결정을 내리려면 `on_approval_request` 콜백을 제공하세요.
...
콜백은 동기 또는 비동기일 수 있으며, 모델이 계속 실행하기 위해 승인 데이터가 필요할 때 호출됩니다.
...
네트워크 연결을 직접 관리하려면
[`MCPServerStreamableHttp`][agents.mcp.server.MCPServerStreamableHttp]를 사용하세요. Streamable HTTP 서버는 전송을 직접 제어하거나, 지연 시간을 낮게 유지하면서 자체 인프라 내에서 서버를 실행하려는 경우에 적합합니다.
...
프로세스를 생성하고 파이프를 연 상태로 유지하며, 컨텍스트 매니저 종료 시 자동으로 닫습니다. 이 옵션은 빠른 프로토타입이나 서버가 커맨드 라인 엔트리 포인트만 노출하는 경우에 유용합니다.
...
각 MCP 서버는 에이전트가 필요한 함수만 노출할 수 있도록 도구 필터를 지원합니다. 필터링은 생성 시점 또는 실행마다 동적으로 수행할 수 있습니다.
...
동기 또는 비동기일 수 있으며, 도구를 노출해야 하는 경우 `True`를 반환합니다.
...
서버 클래스는 `cache_tools_list` 옵션을 제공합니다. 도구 정의가 자주 변경되지 않는다고 확신할 때만 `True`로 설정하세요. 이후에 새 목록을 강제로 가져오려면 서버 인스턴스에서 `invalidate_tools_cache()`를 호출하세요.
