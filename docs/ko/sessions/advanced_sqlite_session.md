...
- **사용량 추적**: 각 턴별 상세 토큰 사용 분석과 전체 JSON 분해
...
AdvancedSQLiteSession 은 대화의 각 턴별 토큰 사용 데이터를 저장하여 상세 사용량 분석을 제공합니다. **이는 각 에이전트 실행 후 `store_run_usage` 메서드가 호출되는지에 전적으로 의존합니다.**
...
AdvancedSQLiteSession 은 기본 SQLite 스키마를 확장하여 두 개의 추가 테이블을 제공합니다:
...
모든 기능을 포괄적으로 보여주는 [완전한 예제](https://github.com/openai/openai-agents-python/tree/main/examples/memory/advanced_sqlite_session_example.py)를 확인하세요.
