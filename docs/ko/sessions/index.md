...
Agents SDK는 에이전트 실행 간 대화 기록을 자동으로 유지하는 세션 메모리를 기본 제공하여, 턴 사이에 `.to_input_list()`를 수동으로 처리할 필요를 없애줍니다.
...
1. **각 실행 이전**: 러너가 세션의 대화 기록을 자동으로 가져와 입력 항목 앞에 추가합니다
2. **각 실행 이후**: 실행 중에 생성된 모든 새 항목(사용자 입력, 어시스턴트 응답, 도구 호출 등)이 자동으로 세션에 저장됩니다
...
SQLAlchemy가 지원하는 어떤 데이터베이스든 사용할 수 있는 프로덕션 준비 완료 세션:
...
대화 분기, 사용량 분석, 구조화된 쿼리를 지원하는 강화된 SQLite 세션:
...
몇 가지 추가 내장 옵션이 있습니다. `examples/memory/`와 `extensions/memory/` 하위의 소스 코드를 참조하세요.
...
30개 이상의 데이터베이스 백엔드, 내장 텔레메트리, 트레이싱, 데이터 격리를 지원
- 기록을 OpenAI Conversations API에 저장하기를 원한다면 OpenAI 호스팅 스토리지(`OpenAIConversationsSession()`) 사용
- 투명한 암호화 및 TTL 기반 만료가 필요하다면 암호화된 세션(`EncryptedSession(session_id, underlying_session, encryption_key)`)으로 어떤 세션이든 래핑
- 더 고급 사용 사례를 위해 다른 프로덕션 시스템(Redis, Django 등)에 대한 커스텀 세션 백엔드를 구현하는 것을 고려
...
세션 구현을 제작하셨다면, 여기 추가될 수 있도록 문서 PR을 기여해 주세요!
...
- [`AdvancedSQLiteSession`][agents.extensions.memory.advanced_sqlite_session.AdvancedSQLiteSession] - 분기 및 분석이 강화된 SQLite
- [`EncryptedSession`][agents.extensions.memory.encrypt_session.EncryptedSession] - 어떤 세션에도 사용할 수 있는 암호화 래퍼