...
`EncryptedSession`은 모든 세션 구현에 대해 투명한 암호화를 제공하며, 오래된 항목을 자동으로 만료 처리하여 대화 데이터를 안전하게 보호합니다.
...
!!! warning "고급 세션 기능"
    ...
    - 콘텐츠 기반 검색은 암호화된 데이터에 대해 수행되어 효과가 제한됩니다



## 키 유도

EncryptedSession은 세션별 고유한 암호화 키를 유도하기 위해 HKDF(HMAC 기반 키 유도 함수)를 사용합니다:
