...
!!! warning "고급 세션 기능"
    ...
    - 메시지 본문이 암호화되므로 `find_turns_by_content()` 같은 메서드는 효과적으로 동작하지 않음
    - 본문 기반 검색은 암호화된 데이터에서 수행되어 효율이 제한됨



## 키 파생

EncryptedSession은 세션별로 고유한 암호화 키를 파생하기 위해 HKDF(HMAC 기반 키 파생 함수)를 사용합니다:
