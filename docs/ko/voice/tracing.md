...
- [`trace_include_sensitive_data`][agents.voice.pipeline_config.VoicePipelineConfig.trace_include_sensitive_data]: 오디오 전사 등 잠재적으로 민감한 데이터의 포함 여부를 제어합니다. 이는 음성 파이프라인에만 해당하며, 귀하의 Workflow 내부에서 발생하는 내용에는 적용되지 않습니다.
