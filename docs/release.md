...
-   Local MCP connections created by the Agents SDK now support MCP Python SDK v2 while retaining v1 compatibility through `mcp>=1.19.0,<3`. The Agents SDK adapts stdio, SSE, and Streamable HTTP connections automatically. Custom HTTP authentication and client factories must use `httpx` with MCP v1 or `httpx2` with MCP v2, and `MCPServerStreamableHttp`'s `params["ignore_initialized_notification_failure"] = True` option remains v1-only. See [MCP Python SDK v1 and v2](mcp.md#mcp-python-sdk-v1-and-v2) for migration details.
-   Sandbox mount validation now rejects unsafe credential placement before sandbox or mount-helper side effects. Serialized sandbox state removes cloud mount authority. For backends that support resuming mounted sessions, the Agents SDK restores credentials only from a current trusted manifest with exactly the same credential-free mount topology. See [Mounts and remote storage](sandbox/clients.md#mounts-and-remote-storage) and [Resume from session state](sandbox/guide.md#resume-from-session-state).
-   Runtime reliability fixes align streamed and non-streamed [output-guardrail session persistence](guardrails.md#output-guardrails), attempt and await [pre-compaction history recovery](sessions/index.md#auto-compaction-can-block-streaming) before cancellation reaches the caller, preserve `FunctionTool` subclasses during copying and namespacing, and raise an explicit error for [unsupported Chat Completions audio output](models/index.md#chat-completions-compatibility-options) instead of silently completing an empty stream.
