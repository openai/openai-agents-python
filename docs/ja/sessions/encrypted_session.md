...
`EncryptedSession` は、あらゆるセッション実装に透過的な暗号化を提供し、自動的な期限切れにより古い項目を保護します。

## 機能

- **透過的な暗号化** : 任意のセッションを Fernet 暗号化でラップします
- **セッションごとのキー** : 一意の暗号化のために HKDF キー導出を使用します
- **自動期限切れ** : TTL を超えた古い項目は、黙ってスキップされます
- **そのまま置き換え可能** : 既存の任意のセッション実装で動作します
...
!!! warning "高度なセッション機能"
    ...
    - 内容ベースの検索は暗号化データに対して実行されるため、有効性が制限されます
