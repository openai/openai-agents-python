...
Agents SDK は、各ランのトークン使用状況を自動的に追跡します。ランのコンテキストからアクセスでき、コストの監視、上限の適用、分析の記録に使えます。
...
セッションはラン間で会話コンテキストを保持しますが、各 `Runner.run()` 呼び出しで返される使用状況メトリクスは、その実行のみを表します。セッションでは、以前のメッセージが各ランの入力として再投入されることがあり、その結果、後続ターンの入力トークン数に影響します。
