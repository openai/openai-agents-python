...
LiteLLM でサポートされているモデルの完全な一覧は、[litellm の providers ドキュメント](https://docs.litellm.ai/docs/providers)をご覧ください。
...
LiteLLM の応答を Agents SDK の使用状況メトリクスに反映させるには、エージェント作成時に `ModelSettings(include_usage=True)` を渡してください。
