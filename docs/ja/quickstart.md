...
追加のエージェントも同じ方法で定義できます。`handoff_descriptions` はハンドオフのルーティングを決定する際の追加コンテキストを提供します。
...
ワークフローが正しく実行され、トリアージ エージェントが 2 つのスペシャリスト エージェント間で正しくルーティングすることを確認しましょう。
...
ハンドオフと入力ガードレールを使用して、すべてを組み合わせた完全なワークフローを実行してみましょう。
...
エージェント実行中に何が起こったか確認するには、[OpenAI Dashboard の Trace viewer](https://platform.openai.com/traces) に移動してエージェントのトレースを表示してください。
