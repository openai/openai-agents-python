...
`compaction_mode="previous_response_id"` works best when you are already chaining turns with Responses API response IDs. `compaction_mode="input"` rebuilds the compaction request from the current session items instead, which is useful when the response chain is unavailable or you want the session contents to be the source of truth. The default `"auto"` uses `previous_response_id` only when the response is usable and its model input is known to cover the full stored session history. When the only shortfall is a `SessionSettings.limit` window, `"auto"` rebuilds the compaction request from the full locally stored history as sanitized input. When a session input callback, model input filter, handoff, or resumed run transforms or hides the effective model input, `"auto"` skips compaction for that turn and preserves the stored history untouched; use an explicit mode to compact anyway. Explicit `"input"` and `"previous_response_id"` modes are unchanged.

If your agent runs with `ModelSettings(store=False)`, the Responses API does not retain the last response for later lookup. In that stateless setup, the default `"auto"` mode falls back to sanitized input-based compaction instead of relying on `previous_response_id` when the effective model input is otherwise untransformed. If a callback, model-input filter, handoff, or resumed run transforms the input, auto mode preserves the store and skips compaction for that turn. See [`examples/memory/compaction_session_stateless_example.py`](https://github.com/openai/openai-agents-python/tree/main/examples/memory/compaction_session_stateless_example.py) for a complete example.
