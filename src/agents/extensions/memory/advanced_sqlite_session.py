...
class AdvancedSQLiteSession(SQLiteSession):
    ...
    async def get_conversation_turns(self, branch_id: str | None = None) -> list[dict[str, Any]]:
        ...
        Returns:
            List of dicts with turn info: {
                'turn': int,               # Branch turn number
                'content': str,            # User message content (truncated)
                'full_content': str,       # Full user message content
                'timestamp': str,          # When the turn was created
                'can_branch': bool         # Always True (all user messages can branch)
            }
        """
        if branch_id is None:
            branch_id = self._current_branch_id
