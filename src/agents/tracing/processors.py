...
def _detach_default_processor() -> TracingExporter | None:
    """Clear the module-owned default processor/exporter singletons and return the exporter.

    The caller (the setup layer, when the default provider shuts down) invokes this while holding
    the global-provider lock and resets ``GLOBAL_TRACE_PROVIDER`` in the same critical section, so
    the default provider, processor, and exporter are released as one unit and a concurrent
    ``get_trace_provider()`` rebuilds a fully fresh stack rather than adopting the just-released
    processor. The returned exporter is closed by ``_close_default_exporter`` once the locks are
    released. The next ``default_processor``/``default_exporter`` call rebuilds the singletons.
    """
    global _global_exporter
    global _global_processor
...
def _close_default_exporter(exporter: TracingExporter | None) -> None:
    """Idempotently close a detached default exporter's HTTP client.

    When the default processor shut down normally it already closed the exporter (``close`` is
    idempotent); this also covers the case where the default processor was replaced -- e.g.
    ``set_trace_processors`` swaps the provider's processors after the default stack was
    bootstrapped -- so it never ran its shutdown, which would otherwise leak the exporter's open
    HTTP client on each rebuild cycle. Called outside any lock, since ``close`` may do I/O.
    """
    if exporter is None:
        return
