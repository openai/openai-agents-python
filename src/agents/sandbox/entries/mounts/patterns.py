...
MountPatternConfig = FuseMountConfig | MountpointMountConfig | RcloneMountConfig
...
class FuseMountPattern(MountPatternBase):
    ...
    async def apply(
        ...
        cache_dir = (
            ...
            else Path(f"/tmp/uc-blobfuse-cache/{session_id.hex}") / account / container
        )
        config_dir = Path(f"/tmp/uc-blobfuse-config/{session_id.hex}")
        ...
        await session.write(config_path, io.BytesIO(config_payload))
...
class RcloneMountPattern(MountPatternBase):
    ...
    def resolve_remote_name(
        ...
        *,
        ...
        return f"uc_{remote_kind}_{session_id}"
    ...
    async def apply(
        ...
        config_dir = Path(f"/tmp/uc-rclone-config/{session_id_str}")
        ...
        await session.write(config_path, io.BytesIO(rclone_config.config_text.encode("utf-8")))
...
MountPattern = Annotated[
    FuseMountPattern | MountpointMountPattern | RcloneMountPattern,
