...
class ErrorCode(str, Enum):
    """Stable, machine-readable error codes for `UniversalComputerError`."""
...
class SandboxError(UniversalComputerError):
    """Raised for sandbox failures (e.g., Docker/IO/transport)."""


class ArtifactError(UniversalComputerError):
    """Raised while materializing input artifacts (local files, git repos)."""


class SnapshotError(UniversalComputerError):
...
class ExecFailureError(SandboxError):
...
class ExecNonZeroError(ExecFailureError):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            message=exec_result.stderr.decode("utf-8", errors="replace"),
...
class WorkspaceIOError(SandboxError):
...
class WorkspaceStopError(SandboxError):
...
class WorkspaceStartError(SandboxError):
...
class WorkspaceRootNotFoundError(SandboxError):
