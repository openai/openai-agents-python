...
class EventSink(abc.ABC):
    """Consumes UCEvent objects (e.g., callback, file outbox, proxy HTTP)."""
...
class CallbackSink(EventSink):
    ...
    def __init__(
        self,
        callback: Callable[[UCEvent, BaseSandboxSession], object],
        *,
...
class ChainedSink(EventSink):
    ...
    Note: Instrumentation unwraps this group and applies per-op/per-sink payload policies to each
    inner sink individually (so grouping does not disable per-sink policy behavior).
    """

    def __init__(self, *sinks: EventSink) -> None:
        self.sinks = list(sinks)
        # These are not used directly when Instrumentation unwraps the group, but keep the object
        # conforming to EventSink.
        self.mode = "sync"
        self.on_error = "raise"
