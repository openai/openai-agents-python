...
from .events import EventPayloadPolicy, UCEvent, UCFinishEvent
...
class Instrumentation:
    ...
    async def emit(self, event: UCEvent) -> None:
    ...
    def _apply_policy(self, event: UCEvent, policy: EventPayloadPolicy) -> UCEvent:
        ...
        if isinstance(out, UCFinishEvent):
    ...
    async def _deliver(self, sink: EventSink, event: UCEvent) -> None:
        async def _run() -> None:
    ...
    async def _deliver_chained(self, sink: EventSink, event: UCEvent) -> None:
        """
    ...
    def _handle_sink_error(
        self, sink: EventSink, event: UCEvent, *, force_no_raise: bool = False
    ) -> None:
        if force_no_raise or sink.on_error in ("log", "ignore"):
            if sink.on_error == "log":
                logger.exception("instrumentation sink failed (ignored): %s", type(sink).__name__)
            return
        raise RuntimeError(
            "instrumentation sink failed: "
