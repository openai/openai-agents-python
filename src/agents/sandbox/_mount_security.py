...
def _raw_manifest_may_require_mount_authority_rebind(payload: object) -> bool:
    """Conservatively identify external mounts whose prior authority cannot be proven absent."""
    ...
    for _path, entry in _iter_raw_entries(
        ...
        if entry_class is None or not issubclass(entry_class, Mount):
            continue
        mount_type = _canonical_mount_type(entry_class)
        ...
        if not isinstance(strategy_type, str):
            continue
        classification = _STRATEGY_CLASSIFICATION_BY_TYPE.get(strategy_type)
        if classification is None or classification[0] != "external":
            continue
        if (
            mount_type in _AUTHORITY_FIELDS_BY_MOUNT_TYPE
            or strategy_type in _OPAQUE_STRATEGY_AUTHORITY_FIELDS
        ):
            return True
    return False
...
def _sanitize_raw_mount(
    ...
    strategy_type = raw_strategy_type if isinstance(raw_strategy_type, str) else ""
    ...
    if serialized_strategy_fields is not None and (
        MountStrategyBase._subclass_registry.get(strategy_type)
        is not _trusted_strategy_class(strategy_type)
    ):
...
def sanitize_raw_session_state_mount_authority(payload: object) -> tuple[object, bool]:
    ...
    if "manifest" in state and not isinstance(state["manifest"], Mapping):
        ...
        _mark_error_data_redacted(error)
        _raise_data_redacted_error(error)
    manifest_may_require_rebind = _raw_manifest_may_require_mount_authority_rebind(
        state.get("manifest")
    )
    ...
    if redacted or state.get(REDACTED_MOUNT_AUTHORITY_KEY) is True or manifest_may_require_rebind:
