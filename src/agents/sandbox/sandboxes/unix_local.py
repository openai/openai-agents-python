...
class UnixLocalSandboxClient(BaseSandboxClient[UnixLocalSandboxClientOptions | None]):
    ...
    async def resume(
        ...
        if not state.provider_identity_trusted:
            if not state.workspace_root_owned:
                raise MountConfigError(
                    message=(
                        "persisted UnixLocal sessions with a caller-owned workspace cannot be "
                        "resumed; create a new session from current trusted configuration"
                    ),
                    context={"backend": "unix_local"},
                )
            fresh_manifest = state.manifest.model_copy(
                update={"root": _DEFAULT_MANIFEST_ROOT},
                deep=True,
            )
            return await self.create(
                snapshot=state.snapshot,
                manifest=fresh_manifest,
                options=UnixLocalSandboxClientOptions(exposed_ports=state.exposed_ports),
            )
        inner = UnixLocalSandboxSession.from_state(state)
        return self._wrap_session(inner, instrumentation=self._instrumentation)
