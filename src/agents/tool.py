...
class FunctionTool:
    ...
    """Short human-readable display label from the originating MCP tool.

    Set by ``MCPUtil.to_function_tool`` so that ``ToolCallItem.title`` can be
    populated without reverse-looking-up the server's cached tool metadata.
    Resolved from ``tool.title`` first, then ``tool.annotations.title``.
    """
