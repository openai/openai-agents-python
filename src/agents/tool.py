...
@dataclass
class ImageFunctionTool:
    """A tool that wraps a function that generates an image. In most cases, you should use  the `image_function_tool` helpers to
    create a ImageFunctionTool, as they let you easily wrap a Python function.
    """
...
def image_function_tool(
    ...
    *,
    ...
    failure_error_function: ToolErrorFunction | None = default_tool_error_function,
    strict_mode: bool = True,
) -> ImageFunctionTool | Callable[[ToolFunction[...]], ImageFunctionTool]:
    """
    Decorator to create a FunctionTool from a function. By default, we will:
    1. Parse the function signature to create a JSON schema for the tool's parameters.
    2. Use the function's docstring to populate the tool's description.
    ...
    def _create_image_function_tool(the_func: ToolFunction[...]) -> ImageFunctionTool:
        schema = function_schema(
            func=the_func,
            name_override=name_override,
            description_override=description_override,
    ...
    # If func is actually a callable, we were used as @function_tool with no parentheses
    if callable(func):
        return _create_image_function_tool(func)

    # Otherwise, we were used as @function_tool(...), so return a decorator
    def decorator(real_func: ToolFunction[...]) -> ImageFunctionTool:
        return _create_image_function_tool(real_func)
