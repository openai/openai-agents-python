    ...
    if pending_write is not None:
        ...
        try:
            validated_pending_write_items = (
                [_HANDOFF_OUTPUT_ADAPTER.validate_python(item) for item in pending_write_items]
                if isinstance(pending_write_items, list)
                else None
            )
            validated_pending_input_write = (
                [_HANDOFF_OUTPUT_ADAPTER.validate_python(item) for item in pending_input_write]
