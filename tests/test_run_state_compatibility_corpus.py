...
def _scoped_approval_payload() -> dict[str, Any]:
    ...
    native_approval = ToolApprovalItem(
        ...
        raw_item={
            ...
            "operation": {"type": "delete_file", "path": "native.txt"},
        },
        tool_name="apply_patch",
    )
    sandbox_approval = ToolApprovalItem(
        agent=agent,
        raw_item={
            "type": "custom_tool_call",
            "call_id": "sandbox-patch",
            "name": "apply_patch",
            "input": "*** Begin Patch\n*** Delete File: sandbox.txt\n*** End Patch\n",
        },
        tool_name="apply_patch",
    )

    state.approve(native_approval, always_approve=True)
    state.reject(
        sandbox_approval,
        always_reject=True,
        rejection_message="Denied by sandbox reviewer",
    )
    context = state._context
    assert context is not None
    sandbox_scope = context._tool_invocations["sandbox-patch"].approval_scope
    scoped_key = f"__scoped_approval__:{sandbox_scope}:apply_patch"
    assert scoped_key in context._approvals
...
def test_historical_fixture_corpus_matches_supported_schema_versions() -> None:
    ...
    # The current writer can introduce a version before a historical writer exists.
    ...
    assert {entry["version"] for entry in SOURCES["features"]} == {
        version for version in SOURCES["versions"] if version not in {"1.0", "1.1"}
    }
...
async def test_legacy_schema_rejects_ambiguous_patch_sticky_before_callbacks(
    schema_version: str, approved: bool
) -> None:
    fixture = FIXTURE_ROOT / "minimal" / f"v{schema_version.replace('.', '_')}.json"
    payload = json.loads(fixture.read_text(encoding="utf-8"))
    payload["context"]["approvals"]["apply_patch"] = {
        "approved": approved,
        "rejected": not approved,
    }
    sentinel = "AMBIGUOUS_PATCH_APPROVAL_SECRET_SENTINEL_42"
    payload["original_input"] = sentinel
    callbacks: list[str] = []

    with pytest.raises(UserError, match="unscoped sticky apply_patch approval") as exc_info:
        await RunState.from_json(
            Agent(name="compat-agent"),
            payload,
            context_deserializer=lambda _context: callbacks.append("context"),
        )
...
@pytest.mark.parametrize("exact_only", [False, True], ids=["sticky", "exact_only"])
async def test_scoped_approval_fixture_preserves_independent_decisions(exact_only: bool) -> None:
    payload = _scoped_approval_payload()
    approvals = payload["context"]["approvals"]
    sandbox_scope = payload["context"]["tool_invocations"]["sandbox-patch"]["approval_scope"]
    sandbox_key = f"__scoped_approval__:{sandbox_scope}:apply_patch"
    sandbox_record = approvals[sandbox_key]
    if exact_only:
        sandbox_record.update({"approved": [], "rejected": ["sandbox-patch"]})
        sandbox_record.pop("sticky_scope")
        sandbox_record.pop("sticky_rejection_message")
    else:
        assert sandbox_record["rejected"] is True
        assert sandbox_record["sticky_scope"] == sandbox_scope

    assert payload["$schemaVersion"] == CURRENT_SCHEMA_VERSION
    assert approvals["apply_patch"]["approved"] is True

    restored = await RunState.from_json(Agent(name="compat-agent"), payload)

    assert restored._context is not None
    assert restored._context.is_tool_approved("apply_patch", "native-patch") is True
    assert restored._context.is_tool_approved("apply_patch", "sandbox-patch") is False
    assert (
        restored._context.get_rejection_message("apply_patch", "sandbox-patch")
        == "Denied by sandbox reviewer"
    )


@pytest.mark.parametrize("schema_version", ["1.15", "1.16", "1.17"])
@pytest.mark.parametrize("native_sticky", [False, True], ids=["exact", "sticky"])
async def test_legacy_mixed_patch_decisions_partition_before_current_write(
    schema_version: str, native_sticky: bool
) -> None:
    payload = _scoped_approval_payload()
    approvals = payload["context"]["approvals"]
    scoped_key = next(key for key in approvals if key != "apply_patch")
    approvals.pop(scoped_key)
    bare_record = approvals["apply_patch"]
    bare_record.update(
        {
            "approved": True if native_sticky else ["native-patch"],
            "rejected": ["sandbox-patch"],
            "rejection_messages": {"sandbox-patch": "Denied by sandbox reviewer"},
        }
    )
    if native_sticky:
        bare_record["sticky_scope"] = payload["context"]["tool_invocations"]["native-patch"][
            "approval_scope"
        ]
    else:
        bare_record.pop("sticky_scope")
    payload["$schemaVersion"] = schema_version

    restored = await RunState.from_json(Agent(name="compat-agent"), payload)
    followup = ToolApprovalItem(
        agent=restored._current_agent,
        raw_item={
            "type": "custom_tool_call",
            "name": "apply_patch",
            "call_id": "sandbox-followup",
            "input": "*** Begin Patch\n*** Delete File: followup.txt\n*** End Patch\n",
        },
        tool_name="apply_patch",
    )
    restored.approve(followup)

    round_tripped = await RunState.from_json(restored._current_agent, restored.to_json())

    assert round_tripped._context is not None
    assert round_tripped._context.is_tool_approved("apply_patch", "native-patch") is True
    assert round_tripped._context.is_tool_approved("apply_patch", "sandbox-patch") is False
    assert round_tripped._context.is_tool_approved("apply_patch", "sandbox-followup") is True


async def test_scoped_approval_fixture_rejects_downgraded_schema_label() -> None:
    payload = _scoped_approval_payload()
    payload["$schemaVersion"] = "1.17"

    with pytest.raises(
        UserError,
        match=r"Invocation-scoped tool approvals require schema version 1\.18 or later",
    ):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "wrong_scope",
        "wrong_exact_scope",
        "wrong_approved_call_scope",
        "wrong_rejected_call_scope",
        "wrong_exact_key_scope",
        "cross_plane_scope_claim",
        "missing_exact_invocation",
        "empty_exact_call_id",
        "malformed_record",
        "malformed_decision",
        "contradictory_sticky",
        "overlapping_exact",
        "bare_contradictory_sticky",
        "bare_overlapping_exact",
        "bare_malformed_decision",
        "bare_empty_call_id",
        "bare_unbound_call_id",
        "bare_wrong_exact_scope",
        "scoped_unhashable_call_id",
        "bare_unhashable_call_id",
        "bare_only_malformed",
        "legacy_bare_only_cross_scope",
    ],
)
async def test_scoped_approval_fixture_rejects_unbound_or_malformed_records(
    invalid_record: str,
) -> None:
    payload = _scoped_approval_payload()
    approvals = payload["context"]["approvals"]
    bare_record = approvals["apply_patch"]
    scoped_key = next(key for key in approvals if key != "apply_patch")
    record = approvals[scoped_key]
    if invalid_record == "bare_only_malformed":
        approvals.pop(scoped_key)
        bare_record["approved"] = "allow"
    elif invalid_record == "legacy_bare_only_cross_scope":
        approvals.pop(scoped_key)
        bare_record.update({"approved": ["native-patch"], "rejected": ["sandbox-patch"]})
        bare_record.pop("sticky_scope")
        payload["context"]["tool_invocations"]["sandbox-patch"]["approval_scope"] = payload[
            "context"
        ]["tool_invocations"]["native-patch"]["approval_scope"]
        payload["$schemaVersion"] = "1.17"
    elif invalid_record == "missing_scope":
        record.pop("sticky_scope")
    elif invalid_record == "wrong_scope":
        record["sticky_scope"] = "0" * 64
    elif invalid_record == "wrong_exact_scope":
        record["approved"] = []
        record["rejected"] = ["sandbox-patch"]
        record["sticky_scope"] = "0" * 64
    elif invalid_record in {
        "wrong_approved_call_scope",
        "wrong_rejected_call_scope",
        "wrong_exact_key_scope",
        "missing_exact_invocation",
        ...
        record.update({"approved": [], "rejected": []})
        record.pop("sticky_scope")
        record.pop("sticky_rejection_message")
        decision = "approved" if invalid_record == "wrong_approved_call_scope" else "rejected"
        call_id = {
            "wrong_approved_call_scope": "native-patch",
            "wrong_rejected_call_scope": "native-patch",
            "wrong_exact_key_scope": "sandbox-patch",
            "missing_exact_invocation": "missing-call",
            "empty_exact_call_id": "",
        }[invalid_record]
        record[decision] = [call_id]
        if invalid_record == "wrong_exact_key_scope":
            approvals[f"__scoped_approval__:{'0' * 64}:apply_patch"] = approvals.pop(scoped_key)
    elif invalid_record == "cross_plane_scope_claim":
        native_scope = payload["context"]["tool_invocations"]["native-patch"]["approval_scope"]
        record["sticky_scope"] = native_scope
        payload["context"]["tool_invocations"]["sandbox-patch"]["approval_scope"] = native_scope
        approvals[f"__scoped_approval__:{native_scope}:apply_patch"] = approvals.pop(scoped_key)
    elif invalid_record == "malformed_decision":
        record["approved"] = "allow"
    elif invalid_record == "contradictory_sticky":
        record["approved"] = True
    elif invalid_record == "overlapping_exact":
        record.update({"approved": ["sandbox-patch"], "rejected": ["sandbox-patch"]})
        record.pop("sticky_scope")
        record.pop("sticky_rejection_message")
    elif invalid_record == "scoped_unhashable_call_id":
        record.update({"approved": [{}], "rejected": []})
        record.pop("sticky_scope")
        record.pop("sticky_rejection_message")
    elif invalid_record.startswith("bare_"):
        if invalid_record == "bare_contradictory_sticky":
            bare_record["rejected"] = True
        elif invalid_record == "bare_malformed_decision":
            bare_record["rejected"] = "deny"
        elif invalid_record == "bare_unhashable_call_id":
            bare_record.update({"approved": [{}], "rejected": []})
            bare_record.pop("sticky_scope")
        else:
            call_id = {
                "bare_overlapping_exact": "native-patch",
                "bare_empty_call_id": "",
                "bare_unbound_call_id": "missing-call",
                "bare_wrong_exact_scope": "native-patch",
            }[invalid_record]
            bare_record.update(
                {
                    "approved": [call_id],
                    "rejected": [call_id] if invalid_record == "bare_overlapping_exact" else [],
                }
            )
            bare_record.pop("sticky_scope")
            if invalid_record == "bare_wrong_exact_scope":
                payload["context"]["tool_invocations"]["native-patch"]["type"] = "custom_tool_call"
    else:
        approvals[scoped_key] = []

    callback_invocations: list[str] = []
    sentinel = "SCOPED_APPROVAL_SECRET_SENTINEL_42"
    payload["original_input"] = sentinel

    with pytest.raises(
        UserError,
        match="RunState scoped tool approval contains invalid invocation scope or decision state",
    ) as exc_info:
        await RunState.from_json(
            Agent(name="compat-agent"),
            payload,
            context_deserializer=lambda _context: callback_invocations.append("context"),
        )
