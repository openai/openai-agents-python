...
async def test_cancel_after_turn_reuses_normalized_history() -> None:
    ...
    model.add_multiple_turn_outputs(
        [
            [
                get_function_tool_call(
                    "fetch_inventory", json.dumps({"sku": "sku-123"}), call_id="call_inventory"
                )
            ],
            [get_text_message("There are 3 new units and 1 refurbished unit available.")],
        ]
    )
    ...
    next_input = streamed.to_input_list(mode="normalized")
    next_input.append(
        {
            "role": "user",
    ...
    resumed = await Runner.run(streamed.last_agent, next_input)

    assert resumed.final_output == "There are 3 new units and 1 refurbished unit available."

    resumed_input = cast(list[dict[str, Any]], model.last_turn_args["input"])
    assert resumed_input[0] == {"content": "Check warehouse stock for sku-123.", "role": "user"}
    assert any(
        isinstance(item, dict)
        and item.get("type") == "function_call_output"
        and item.get("call_id") == "call_inventory"
        for item in resumed_input
    )
    assert resumed_input[-1] == {
        "content": "Also include refurbished inventory if available.",
        "role": "user",
    }
