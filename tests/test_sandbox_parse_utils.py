...
def test_parse_ls_la_preserves_absolute_file_paths() -> None:
    output = "-rwxr-xr-x 1 root root 48915747 Jan 1 00:00 /workspace/.codex_bin/codex\n"

    entries = parse_ls_la(output, base="/workspace/.codex_bin/codex")

    assert len(entries) == 1
    assert entries[0].path == "/workspace/.codex_bin/codex"
