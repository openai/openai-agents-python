...
import pytest
from pydantic import ConfigDict, PrivateAttr, TypeAdapter, model_serializer, model_validator
...
def test_session_state_deserialization_recovers_removed_mount_authority_marker() -> None:
    manifest = Manifest(
        entries={
            "data": _s3_mount(
                strategy=DockerVolumeMountStrategy(
    ...
    client = _SecurityTestClient()
    payload = client.serialize_session_state(state)
    payload.pop(REDACTED_MOUNT_AUTHORITY_KEY)
    payload[CREDENTIALLESS_MOUNT_AUTHORITY_KEY] = True
...
def test_session_state_round_trip_requires_credentialless_external_mount_rebind() -> None:
    manifest = Manifest(
        entries={
            "data": S3Mount(
                bucket="example-bucket",
...
def test_generic_session_state_parser_sanitizes_legacy_mount_authority() -> None:
    ...
    assert restored.mount_authority_redacted is True
    assert restored.provider_identity_trusted is False
...
def test_serialization_never_trusts_provider_identity() -> None:
    direct = TestSessionState(
        manifest=Manifest(),
        snapshot=NoopSnapshot(id="snapshot"),
    )
    direct._mark_provider_identity_trusted()
    payload = direct.model_dump(mode="json")
    restored = TestSessionState.model_validate(payload)
    restored_json = TestSessionState.model_validate_json(json.dumps(payload))
    restored_strings = TestSessionState.model_validate_strings(payload)
    adapter = TypeAdapter(TestSessionState)
    restored_adapter = adapter.validate_python(payload)
    restored_adapter_json = adapter.validate_json(json.dumps(payload))
    restored_adapter_strings = adapter.validate_strings(payload)

    assert direct.provider_identity_trusted is True
    assert restored.provider_identity_trusted is False
    assert restored_json.provider_identity_trusted is False
    assert restored_strings.provider_identity_trusted is False
    assert restored_adapter.provider_identity_trusted is False
    assert restored_adapter_json.provider_identity_trusted is False
    assert restored_adapter_strings.provider_identity_trusted is False
