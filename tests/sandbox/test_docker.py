...
async def test_docker_resume_removes_replacement_volumes_when_wrapping_fails(
    ...
    assert state.session_id == session_id
    assert state.workspace_root_ready is True
    assert state.provider_identity_trusted is False
...
async def test_docker_resume_preserves_workspace_readiness_from_state() -> None:
    ...
    ready_state._mark_provider_identity_trusted()
    ...
    not_ready_state._mark_provider_identity_trusted()
...
@pytest.mark.asyncio
async def test_docker_resume_recreates_trusted_credentialless_external_mount(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manifest = Manifest(
        entries={
    ...
    resumed = await client.resume(rebound)

    assert isinstance(resumed._inner, DockerSandboxSession)
    assert rebound.mount_authority_redacted is False
    assert rebound.mount_authority_rebound is True
    assert rebound.session_id == replacement_session_id
    assert rebound.container_id == replacement.id
...
async def test_docker_resume_does_not_reconnect_identity_from_tampered_state(
    ...
    assert restored.provider_identity_trusted is True
    assert restored.container_id == replacement.id
    assert restored.session_id == replacement_session_id
    assert isinstance(resumed._inner, DockerSandboxSession)
