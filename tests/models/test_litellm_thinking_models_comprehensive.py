...
class TestLiteLLMThinkingModels:
    ...
    async def test_reproduce_original_error_with_mock(self):
        ...
        with patch("litellm.acompletion", new=mock_acompletion):
            ...
            assert "When `thinking` is enabled, a final `assistant` message must start with a thinking block" in error_message
    ...
    async def test_real_api_thinking_model_current_state(self):
        """
        Test the current state of thinking models with the real API.
        
        This test documents the progress made on fixing issue #765:
        - Original error: "Expected `thinking` or `redacted_thinking`, but found `text`"
        - Current error: "Expected `thinking` or `redacted_thinking`, but found `tool_use`"
        
        This shows our fix is working partially - we've eliminated the text issue,
        but LiteLLM is still converting tool_calls to tool_use content blocks
        without proper thinking blocks.
        """
        count_ctx = Count(count=0)
        ...
        # Test current state - should show progress from original issue
        ...
        # Verify we've made progress from the original issue
        if "found `tool_use`" in error_str:
            # Expected progress - we've fixed the 'found text' issue
            print("✓ Progress: Fixed the 'found text' issue, now dealing with tool_use format")
        elif "found `text`" in error_str:
            # Regression - we're back to the original error
            pytest.fail("Regression: Back to the original 'found text' error")
        else:
            # Could be success or a different error
            print(f"Unexpected result: {error_str}")
    ...
    async def test_real_api_reproduction_simple(self):
        ...
        try:
            ...
            if "Expected `thinking` or `redacted_thinking`" in error_str:
                if "found `tool_use`" in error_str:
                    print("Current state: Partial fix - eliminated 'text' error, working on 'tool_use'")
                elif "found `text`" in error_str:
