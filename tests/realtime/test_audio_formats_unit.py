...
def test_to_realtime_audio_format_from_strings():
    assert to_realtime_audio_format("pcm").type == "audio/pcm"
    assert to_realtime_audio_format("pcm16").type == "audio/pcm"
    assert to_realtime_audio_format("audio/pcm").type == "audio/pcm"
    assert to_realtime_audio_format("pcmu").type == "audio/pcmu"
    assert to_realtime_audio_format("audio/pcmu").type == "audio/pcmu"
    assert to_realtime_audio_format("g711_ulaw").type == "audio/pcmu"
    assert to_realtime_audio_format("pcma").type == "audio/pcma"
    assert to_realtime_audio_format("audio/pcma").type == "audio/pcma"
    assert to_realtime_audio_format("g711_alaw").type == "audio/pcma"
