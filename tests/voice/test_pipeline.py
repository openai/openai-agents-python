...
async def test_voicepipeline_accepts_numpy_dtype_spellings(
    ...
    The string cases are the ones that fail before this change, and the aliases hold the
    property the fix rests on: the value is resolved the way NumPy resolves it rather than
    matched against a fixed set of names. The resolved-dtype / type-object cases pin spellings
