...
@pytest.mark.asyncio
async def test_modal_resume_recreates_deserialized_credentialless_external_mount(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    modal_module, create_calls, _registry_tags = _load_modal_module(monkeypatch)
...
@pytest.mark.asyncio
async def test_modal_resume_recreates_generically_parsed_rebound_mount(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    modal_module, create_calls, _registry_tags = _load_modal_module(monkeypatch)
    trusted_manifest = Manifest(
...
@pytest.mark.asyncio
async def test_modal_resume_rejects_rebound_mount_authority_before_reconnect(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    modal_module, create_calls, _registry_tags = _load_modal_module(monkeypatch)
    trusted_manifest = Manifest(
    ...
    with pytest.raises(
        MountConfigError,
        match="protected volume configuration cannot be resumed",
    ):
